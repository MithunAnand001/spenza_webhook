import { WebhookEventStatus, SubscriptionStatus } from '../constants/status';
...
export default function StatusBadge({ status }: { status: AllStatuses }) {
  const colors: Record<AllStatuses, string> = {
    ...
    [SubscriptionStatus.ACTIVE]: 'bg-indigo-100 text-indigo-800 border-indigo-200',
    [SubscriptionStatus.INACTIVE]: 'bg-slate-100 text-slate-800 border-slate-200',
  };
