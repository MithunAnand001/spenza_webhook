...
export interface ISubscriptionsService {
  ...
  getSubscription(userId: number, id: number): Promise<UserEventMapping>;
  cancelSubscription(userId: number, id: number): Promise<UserEventMapping>;
}

export interface IWebhookIngestService {
  ingest(subscriptionId: number, payload: any, signature?: string, correlationId?: string): Promise<any>;
}
